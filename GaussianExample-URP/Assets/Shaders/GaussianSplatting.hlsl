...
StructuredBuffer<int> _SplatLabels;
int _RenderMode;

float3 HashLabelToColor(int label)
{
    // simple hash (same as Unity's editor handles)
    uint h = (uint)label * 747796405u + 2891336453u;
    h = (h >> 13) ^ h;
    float r = ((h & 0xFFu)     ) / 255.0;
    float g = ((h & 0xFF00u) >> 8) / 255.0;
    float b = ((h & 0xFF0000u) >> 16) / 255.0;
    return float3(r, g, b);
}
