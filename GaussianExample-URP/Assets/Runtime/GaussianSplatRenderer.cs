...
namespace GaussianSplatting.Runtime
{
    class GaussianSplatRenderSystem
    {
        ...
        public Material SortAndRenderSplats(Camera cam, CommandBuffer cmb)
        {
            ...
            foreach (var kvp in m_ActiveSplats)
            {
                ...
                // 🔹 make ColorByLabel reachable in the shader
                mpb.SetInteger(GaussianSplatRenderer.Props.RenderMode, (int)gs.m_RenderMode);

                // 🔹 ensure labels buffer is bound on the *same* MPB that draws
                if (gs.SplatLabels != null)
                    mpb.SetBuffer(GaussianSplatRenderer.Props.SplatLabels, gs.SplatLabels);
    ...
    {
        // Play-mode kernels (added for shortcuts)
        int m_KernelPlayClearAll;
        int m_KernelPlaySelectAll;
        int m_KernelPlayInvertAll;
        int m_KernelPlayDeleteAll;

        
        ComputeBuffer _SplatLabels;
        public ComputeBuffer SplatLabels => _SplatLabels; // public getter if others need access

        // === Segmentation accessors (added for SplatSegmentationManager) ===
        public GraphicsBuffer GetPosBuffer()     => m_GpuPosData;
        public GraphicsBuffer GetOtherBuffer() => m_GpuOtherData;
        public Texture        GetColorTexture()  => m_GpuColorData;


        // Keep track of label count (1 int per splat)
        int[] _labelsCPU; // optional CPU mirror for debugging/interop
        ...
        [Range(0, 3)] public int m_SHOrder = 3;
        ...
        public int editSelectedSplats;
        public int editDeletedSplats;
        public int editCutSplats;
        public bool editModified;
        public Bounds editSelectedBounds;

        // Convenience aliases (Aras used these)
        public GaussianSplatAsset asset => m_Asset;
        public int splatCount => m_SplatCount;
        // Public access for runtime selector
        public GraphicsBuffer GpuEditSelected => m_GpuEditSelected;
        public GraphicsBuffer GpuEditDeleted  => m_GpuEditDeleted;
        GraphicsBuffer _SplatIDBuffer;
        ...
        internal static class Props
        {
            ...
            public static readonly int WorldToClip = Shader.PropertyToID("_WorldToClip");
            ...
            public static readonly int SplatLabels = Shader.PropertyToID("_SplatLabels");
            public static readonly int RenderMode = Shader.PropertyToID("_RenderMode");
            public static readonly int SplatIDBuffer = Shader.PropertyToID("_SplatIDBuffer");
            public static readonly int LabelPaletteTex   = Shader.PropertyToID("_LabelPaletteTex");
            public static readonly int LabelPaletteCount = Shader.PropertyToID("_LabelPaletteCount");
        ...
        void CreateResourcesForAsset()
        {
            ...
            if (_SplatLabels != null) _SplatLabels.Release();
            _SplatLabels = new ComputeBuffer(m_SplatCount, sizeof(int)) { name = "GaussianSplatLabels" };

            _labelsCPU = new int[m_SplatCount];
            for (int i = 0; i < m_SplatCount; i++) _labelsCPU[i] = -1;
            _SplatLabels.SetData(_labelsCPU);
            // Stable ID buffer: [0, 1, 2, ..., splatCount-1]
            _SplatIDBuffer?.Dispose();
            _SplatIDBuffer = new GraphicsBuffer(GraphicsBuffer.Target.Structured, m_SplatCount, sizeof(int)) { name = "GaussianSplatIDs" };
            int[] ids = new int[m_SplatCount];
            for (int i = 0; i < m_SplatCount; i++) ids[i] = i;
            _SplatIDBuffer.SetData(ids);
        ...
        [ContextMenu("DebugAssignAllLabels")]
        public void DebugAssignAllLabels()
        {
            if (_labelsCPU == null || _SplatLabels == null)
            {
                Debug.LogWarning("Label buffers not initialized.");
                return;
            }

            // Cycle labels in big blocks so it's obvious
            int blockSize = 100000; // 100k per color
            int numLabels = 5;      // 5 unique colors

            for (int i = 0; i < _labelsCPU.Length; i++)
            {
                _labelsCPU[i] = (i / blockSize) % numLabels;
            }

            _SplatLabels.SetData(_labelsCPU);
            Debug.Log($"Assigned ALL splats into {numLabels} label groups (block size {blockSize}).");
        }
        ...
        internal void SetAssetDataOnMaterial(MaterialPropertyBlock mat)
        {
            ...
            if (_SplatLabels != null)
                mat.SetBuffer(Props.SplatLabels, _SplatLabels);

            if (_SplatIDBuffer != null)
                mat.SetBuffer(Props.SplatIDBuffer, _SplatIDBuffer);
                
            // forward optional palette to the material (if set)
            var palTex = Shader.GetGlobalTexture(Props.LabelPaletteTex);
            if (palTex != null)
            {
                mat.SetTexture(Props.LabelPaletteTex, palTex);
                mat.SetInteger(Props.LabelPaletteCount, Shader.GetGlobalInt(Props.LabelPaletteCount));
            }
        ...
        public void EditUpdateSelection(Vector2 rectMin, Vector2 rectMax, Camera cam, bool subtract)
        {
            ...
            var tr   = transform;
            var matV = cam.worldToCameraMatrix;
            var matO2W = tr.localToWorldMatrix;
            var matW2O = tr.worldToLocalMatrix;

            // IMPORTANT: use GPU projection (handles render pipeline/platform flips)
            Matrix4x4 worldToClip = GL.GetGPUProjectionMatrix(cam.projectionMatrix, true) * matV;
            ...
            cmb.SetComputeMatrixParam(m_CSSplatUtilities, Props.WorldToClip, worldToClip);
            cmb.SetComputeMatrixParam(m_CSSplatUtilities, Props.MatrixMV, matV * matO2W);
            cmb.SetComputeMatrixParam(m_CSSplatUtilities, Props.MatrixObjectToWorld, matO2W);
            cmb.SetComputeMatrixParam(m_CSSplatUtilities, Props.MatrixWorldToObject, matW2O);
            ...
            // Pass rect in *bottom-left* origin to match our kernel (no Y flip)
            cmb.SetComputeVectorParam(m_CSSplatUtilities, "_SelectionRect",
                new Vector4(rectMin.x, rectMin.y, rectMax.x, rectMax.y));
...
public void DebugLogSelectionCounts()
{
    Debug.Log($"Selected: {editSelectedSplats}, Deleted: {editDeletedSplats}, Cut: {editCutSplats}, Modified: {editModified}, Bounds: {editSelectedBounds}");
}


// now you are back at class scope
void DispatchPlayKernel(CommandBuffer cmb, int kernelIndex, int count)
{
    m_CSSplatUtilities.GetKernelThreadGroupSizes(kernelIndex, out uint gsX, out _, out _);
    cmb.DispatchCompute(m_CSSplatUtilities, kernelIndex, (count + (int)gsX - 1)/(int)gsX, 1, 1);
    Graphics.ExecuteCommandBuffer(cmb);
}
        public void EditPlayClearSelection()
        {
            if (!EnsureEditingBuffers()) return;
            int wordCount = (m_SplatCount + 31) / 32;
            using var cmb = new CommandBuffer { name = "PlayClearAll" };
            cmb.SetComputeBufferParam(m_CSSplatUtilities, m_KernelPlayClearAll, Props.SplatSelectedBits, m_GpuEditSelected);
            DispatchPlayKernel(cmb, m_KernelPlayClearAll, wordCount);
        }

        public void EditPlaySelectAll()
        {
            if (!EnsureEditingBuffers()) return;
            int wordCount = (m_SplatCount + 31) / 32;
            using var cmb = new CommandBuffer { name = "PlaySelectAll" };
            cmb.SetComputeBufferParam(m_CSSplatUtilities, m_KernelPlaySelectAll, Props.SplatSelectedBits, m_GpuEditSelected);
            DispatchPlayKernel(cmb, m_KernelPlaySelectAll, wordCount);
        }

        public void EditPlayInvertSelection()
        {
            if (!EnsureEditingBuffers()) return;
            int wordCount = (m_SplatCount + 31) / 32;
            using var cmb = new CommandBuffer { name = "PlayInvertAll" };
            cmb.SetComputeBufferParam(m_CSSplatUtilities, m_KernelPlayInvertAll, Props.SplatSelectedBits, m_GpuEditSelected);
            DispatchPlayKernel(cmb, m_KernelPlayInvertAll, wordCount);
        }

        public void EditPlayDeleteSelection()
        {
            if (!EnsureEditingBuffers()) return;
            int wordCount = (m_SplatCount + 31) / 32;
            using var cmb = new CommandBuffer { name = "PlayDeleteAll" };
            cmb.SetComputeBufferParam(m_CSSplatUtilities, m_KernelPlayDeleteAll, Props.SplatSelectedBits, m_GpuEditSelected);
            cmb.SetComputeBufferParam(m_CSSplatUtilities, m_KernelPlayDeleteAll, Props.SplatDeletedBits, m_GpuEditDeleted);
            DispatchPlayKernel(cmb, m_KernelPlayDeleteAll, wordCount);
        }

        public void EditPlayUndeleteSelection()
        {
            if (!EnsureEditingBuffers()) return;
            ClearGraphicsBuffer(m_GpuEditDeleted);
        }
